...
static void init_codel_params(struct QueueCodel* queue)
{
    ...
    queue->pTimer.deq_pos = 0;
    queue->pTimer.enq_pos = 0;
...
static int64_t control_law(struct QueueCodel* queue, int64_t t, uint32_t count)
{
    ...
    printf("control law , now = %lu and new time =  %lu \n", t, t+(int64_t)temp );
...
static struct dodequeue_result dodequeue(struct QueueCodel* queue, int64_t now)
{
    ...
    int64_t sojourn_time = now - queue->pTimer.usecs[queue->pTimer.deq_pos];

    if( queue->pTimer.deq_pos == MAX_NUM_PACKETS - 1)
        queue->pTimer.deq_pos = 0;
    else 
        queue->pTimer.deq_pos++; 
...
void* mib_queue_codel_deque(struct QueueCodel* queue)
{
    ...
    if (queue->dropping_ == 1) {
        ...
        while (now >= queue->drop_next_ && (queue->dropping_ == 1)) {
            ...
            printf("dropping from while in codel at now = %ld and drop_next = %ld \n", now, queue->drop_next_);
...
void mib_queue_codel_enqueu(struct QueueCodel* queue, void* data)
{
    ...
    if(queue->pTimer.enq_pos == MAX_NUM_PACKETS)
        queue->pTimer.enq_pos = 0;

    queue->pTimer.usecs[queue->pTimer.enq_pos] = mib_get_time_us();
    queue->pTimer.enq_pos = queue->pTimer.enq_pos + 1;
}
